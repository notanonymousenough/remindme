...
def habit_illustration_prompt(
    ...
    if completion_rate == 0:
        mood_description = f"Настроение: {animal} недовольный и раздраженный, но всё равно выполняет действие."
    elif completion_rate < 0.5:
        mood_description = f"Настроение: {animal} неуверенный и неуклюжий, но старается правильно выполнить действие."
    elif completion_rate < 0.9:
        mood_description = f"Настроение: {animal} уверенный и довольный, выполняет действие с удовольствием."
    else:
        mood_description = f"Настроение: {animal} как профессионал, выполняет действие идеально и с энтузиазмом."

    # Вторичные инструкции
    secondary_instruction = f"""
{mood_description}
Стиль: мультяшный, яркий, с чёткими контурами.
НЕ ПОКАЗЫВАТЬ: просто портрет {animal}а без действия; {animal}а, который смотрит на предметы, но не использует их; {animal}а в человеческой одежде.
"""
