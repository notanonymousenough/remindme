...
reminder_route = Router()


@reminder_route.callback_query(StateFilter(States.reminder_menu), F.data.startswith("reminder_next_"))
...
@reminder_route.callback_query(StateFilter(States.reminder_menu))
async def show_reminder(call: CallbackData, state: FSMContext):
    pass


@reminder_route.message(StateFilter(States.reminder_menu), F.text == "Фильтрация по тэгам")
async def reminder_tags(message: Message, state: FSMContext):
    await message.answer(text="134134")


@reminder_route.message(StateFilter(States.reminder_menu), F.text == "Добавить напоминание")
async def add_reminder(message: Message, state: FSMContext):
    pass


@reminder_route.message(StateFilter(States.reminder_menu), F.text == "Назад")
